	...
	private void criarComponentes() {
		...
		if (registrationModalityToEdition != null) {
			Modality selectedModality = modalitiesList
					.stream()
					.filter(item -> {
						return item.getId() == registrationModalityToEdition.getModality().getId();
					})
		...
		/*
		 * btnExcluir = new JButton("Excluir"); btnExcluir.setBounds(10, 160, 100, 25);
		 ...
		 * btnExcluir.addActionListener(new ActionListener() { public void
		 * actionPerformed(ActionEvent e) { //Seta o atributo que define a modalidade
		 * como excluida selectedRegistrationModality = new RegistrationModality();
		 * selectedRegistrationModality.setDeleted(true);
		 * 
		 * //Fecha a janela try { setClosed(true); } catch (PropertyVetoException error)
		 * { error.printStackTrace(); } } }); getContentPane().add(btnExcluir);
		 */
	}
			...
			try {
				...
				if (registrationModalityToEdition != null) {
					Graduation selectedGraduation = graduationsList
							.stream()
							.filter(item -> {
								return item.getId() == registrationModalityToEdition.getGraduation().getId();
							})
							.findFirst().orElse(null);
				...
				if (registrationModalityToEdition != null) {
					Plan selectedPlan = plansList
							.stream()
							.filter(item -> {
								return item.getId() == registrationModalityToEdition.getPlan().getId();
							})
							.findFirst().orElse(null);
